...
def parse(datafile, vertex):
    adj_list = []
    
    with open(datafile) as f:
        num_vertices = map(int, f.readline().split())
...
def find_maxdeg(g):
    ...
    deglist_sorted = sorted(deglist.items(), reverse=True, key=operator.itemgetter(1))
...
def main(inputfile, vertex):
    ...
    adj_list = parse(inputfile, vertex)

    for j in range(vertex):
        adj_list[j] = [ele for ele in adj_list[j] if ele <=vertex]
    ...
    print('Solution VC:', VC_Size(Sol_VC))
